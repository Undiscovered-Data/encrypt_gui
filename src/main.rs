...
fn main() {
    ...
    win2.set_color(Color::from_hex(0x2269fd));
    ...
    win3.set_color(Color::from_hex(0x2269fd));
    win3.make_resizable(true);

    //let mut flex21 = Flex::new(10, 10, 260, 160, None);
    //flex21.set_type(group::FlexType::Column);
    let mut label11 = frame::Frame::new(80, 5, 120, 30, "Input file");
    label11.set_label_size(30);
    let mut output = output::Output::new(50, 30, 180, 10, None);
    ...
    let mut label12 = frame::Frame::new(80, 65, 120, 30, "Output file");
    label12.set_label_size(30);
    let mut output2 = output::Output::new(50, 90, 180, 10, None);
    ...
    let mut but5 = button::Button::default()
        .with_size(80, 25)
        .with_label("Do it")
        .with_pos(100, 140);
...
//*********              function callback              ********\\
    ...
    but3.set_callback(move |_| {
        ...
        dialog.show();
        in_file = format!("{:?}", &dialog.filename());
        println!("{}", &in_file);
    });
    //println!("{}", &in_file);  This is not allowed
    a.run().unwrap();
}
